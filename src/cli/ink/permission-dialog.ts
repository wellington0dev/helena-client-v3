...
/**
 * Instância do `Dialog` genérico (ver dialog.ts) — "s"/"y"/"n" são atalhos ESPECÍFICOS deste diálogo (não fazem
 * parte do `Dialog` genérico), então ganham `useInput` próprio aqui. Nunca colidem com o que o `Dialog` já trata
 * (↑↓/Tab/Enter/dígito/Esc) — teclas disjuntas, os dois hooks convivem sem disputar a mesma tecla.
 */
export function PermissionDialog(props: { pending: PendingConfirmation; onAnswer: (decision: PermissionDecision) => void }): React.ReactElement {
    ...
    useInput((input: string) => {
        ...
        if (lower === "n") props.onAnswer("reject");
        else if (lower === "s" || lower === "y") props.onAnswer("once");
    });

    return h(Dialog<PermissionDecision>, {
        title: view.title,
        primary: view.primary,
        context: view.context || undefined,
        question: view.question,
        options: view.options.map((o) => ({ value: o.decision, label: o.label })),
        hint: view.hint,
        onAnswer: props.onAnswer,
        onCancel: () => props.onAnswer("reject"),
    });
}
