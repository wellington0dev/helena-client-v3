/**
 * Duplicado (não importado) de
 * `panel-app/src/app/pages/mcp-guide/mcp-guide-content.ts` — painel e
 * `cli/` são pacotes/builds diferentes hoje, e criar um acoplamento novo
 * entre eles só pra isto não vale a pena. Mesmo conteúdo, adaptado de
 * `backend-v2/docs/mcp-server-guide.md` (+ intro de `mcp-plugins.md`).
 */
...
Use o \`/integracoes\` do terminal pra cadastrar/editar/remover conexões — o guia abaixo é só sobre como CONSTRUIR o servidor MCP em si (o "seu-servidor.exemplo.com/mcp" que você vai colar lá).
...
O jeito mais simples é o \`/integracoes\` no terminal. Se preferir automatizar (scripts de setup, CI, etc), os comandos abaixo já vêm com a URL da sua Helena e o seu token de agora preenchidos — é só trocar \`name\`/\`serverUrl\`/\`authToken\` pelos do seu servidor.
...
${jwt ? "" : "> Faça login pra ver seu token preenchido automaticamente aqui.\n\n"}⚠️ Esse token é o mesmo usado nesta sessão — não compartilhe com ninguém: quem tiver esse token consegue agir como você na Helena.
